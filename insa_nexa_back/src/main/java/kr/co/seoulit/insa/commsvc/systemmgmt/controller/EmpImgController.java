...
public class EmpImgController {
	...
	public ModelMap handleRequestInternal(@RequestAttribute("reqData") PlatformData reqData) {
		...
		try {
			...
			Image resizeImage = image.getScaledInstance( 225, 225, Image.SCALE_SMOOTH); // 이미지의 크기를 조정
			BufferedImage newImage = new BufferedImage( 225, 225, BufferedImage.TYPE_INT_RGB ); // 8 비트 RGB 색상을 사용하는 이미지
